...
const S={user:null,fuel:'SP95-E10',pos:{lat:46.6,lng:1.88},radius:10,stations:JSON.parse(JSON.stringify(DEMO)),comments:[],proposals:[],activity:[],selId:null,map:null,markers:[],userDot:null,userAccuracy:null};

const toast=(m,d=2200)=>{const t=document.getElementById('toast');if(!t)return;t.textContent=m;t.style.opacity=1;clearTimeout(t._x);t._x=setTimeout(()=>t.style.opacity=0,d)};
const now=()=>new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'});
const km=(a,b,c,d)=>{const R=6371,x=(c-a)*Math.PI/180,y=(d-b)*Math.PI/180,z=Math.sin(x/2)**2+Math.cos(a*Math.PI/180)*Math.cos(c*Math.PI/180)*Math.sin(y/2)**2;return +(2*R*Math.atan2(Math.sqrt(z),Math.sqrt(1-z))).toFixed(1)};
...
function filtered(){
  const f=S.fuel,q=(document.getElementById('searchFilter')?.value||'').toLowerCase(),mx=parseFloat(document.getElementById('maxPriceFilter')?.value||'999'),so=document.getElementById('sortFilter')?.value||'price';
  return S.stations.filter(s=>{
    ...
    if(S.radius){const d=km(S.pos.lat,S.pos.lng,s.lat,s.lng);if(d>S.radius)return false}
    return true;
  }).sort((a,b)=>so==='name'?a.name.localeCompare(b.name):so==='distance'?km(S.pos.lat,S.pos.lng,a.lat,a.lng)-km(S.pos.lat,S.pos.lng,b.lat,b.lng):a.prices[f]-b.prices[f]);
}
...
function stationIcon(color){
  return L.divIcon({html:`<svg xmlns="http://www.w3.org/2000/svg" width="28" height="36" viewBox="0 0 28 36"><path d="M14 0C6.3 0 0 6.3 0 14c0 10.2 14 22 14 22s14-11.8 14-22C28 6.3 21.7 0 14 0z" fill="${color}"/><circle cx="14" cy="14" r="6" fill="white" opacity=".92"/></svg>`,className:'',iconSize:[28,36],iconAnchor:[14,36],popupAnchor:[0,-36]});
}
...
function renderDetail(){
  ...
  if(!st){box.innerHTML='<div class="empty"><h3>Choisissez une station</h3><p>Le détail s\'affichera ici.</p></div>';return}
  ...
  FUELS.forEach(f=>{if(st.prices[f]){const cls=st.prices[f]===min?'best':st.prices[f]===max?'worst':'';html+=`<div class="price-chip ${cls}"><div class="f-name">${f}</div><div class="f-val" style="color:${FC[f]}">${st.prices[f].toFixed(3)} €</div></div>`}});
  html+='</div>';
  box.innerHTML=html;
}
...
function renderAdmin(){
  ...
  document.getElementById('adminQueueCount').textContent=q.length;
  ...
  document.getElementById('adminQueueFull').innerHTML=html||'<div class="empty"><h3>File vide</h3><p>Aucune contribution en attente.</p></div>';
  document.getElementById('activityLog').innerHTML=S.activity.map(x=>`<div class="log-item" style="display:flex;justify-content:space-between;gap:.75rem"><div>${x.msg}</div><div style="color:var(--tx2);font-size:.72rem;white-space:nowrap">${x.at}</div></div>`).join('')||'<div class="empty"><h3>Aucune activité</h3></div>';
}

function renderStationList(){
  const fuel=S.fuel,items=filtered(),list=document.getElementById('stationList'),best=items.length?Math.min(...items.map(s=>s.prices[fuel])):null,worst=items.length?Math.max(...items.map(s=>s.prices[fuel])):null;
  document.getElementById('bestPriceMini').textContent=best!==null?`${best.toFixed(3)}€ min`:'—';
  list.innerHTML=items.length?items.map((st,i)=>{
    const price=st.prices[fuel],klass=price===best?'good':price===worst?'high':'mid',dist=km(S.pos.lat,S.pos.lng,st.lat,st.lng);
  ...
  list.querySelectorAll('.station-item').forEach(el=>el.addEventListener('click',()=>{S.selId=+el.dataset.id;renderDetail()}));
}

function renderAll(){hydrateSelects();renderMap();renderDetail();renderSession();renderAdmin();renderStationList();document.getElementById('nFuelBadge').textContent=S.fuel}

function approve(type,id){const c=S.comments.find(x=>x.id===id),p=S.proposals.find(x=>x.id===id);if(type==='comment'&&c)c.status='approved';if(type==='proposal'&&p){p.status='approved';const st=S.stations.find(s=>s.id===p.stId);if(st)st.prices[p.fuel]=p.price}renderAll();toast('Approuvé')}
function reject(type,id){const c=S.comments.find(x=>x.id===id),p=S.proposals.find(x=>x.id===id);if(type==='comment'&&c)c.status='rejected';if(type==='proposal'&&p)p.status='rejected';renderAll();toast('Refusé')}
...
function view(id){
  ...
  const mapPanel=document.getElementById('mapPanel'), adminPanel=document.getElementById('adminPanel');
  if(id==='adminView'){mapPanel.style.display='none';adminPanel.style.display='block';setTimeout(()=>renderAdmin(),50)}
  else{mapPanel.style.display='block';adminPanel.style.display='none'}
...
document.getElementById('cityBtn').addEventListener('click',()=>{
  ...
  if(box.style.display==='block')setTimeout(()=>input.focus(),20);
});

document.getElementById('cityInput').addEventListener('input',()=>{
  const q=document.getElementById('cityInput').value.toLowerCase().trim();
...
document.getElementById('aroundMeBtn').addEventListener('click',()=>{
  const menu=document.getElementById('aroundMenu');
  menu.style.display=menu.style.display==='block'?'none':'block';
});

document.getElementById('radiusRange').addEventListener('input',()=>{
  S.radius=+document.getElementById('radiusRange').value;
...
if(seedBtn){
  seedBtn.addEventListener('click',()=>{
    ...
    S.comments=[{id:1,stId:2,author:'Lucas',text:'Prix conformes ce matin.',status:'approved',at:Date.now()-3e5},{id:2,stId:1,author:'Nora',text:'Beaucoup de monde mais prix corrects.',status:'pending',at:Date.now()-1e5}];
